...
from dataset.mydataset_rec import thirdAugDataset
...
parser.add_argument('--real_datapath', type=str,
                    default='/lab/kirito/data/cliptrain/train/0_real_png.csv')
parser.add_argument('--realrec_datapath', type=str,
                    default='/lab/kirito/data/cliptrain/train/1_real_noisy_png.csv')
parser.add_argument('--fake_datapath', type=str,
                    default='/lab/kirito/data/cliptrain/train/1_fake.csv')
parser.add_argument('--fakerec_datapath', type=str,
                    default='/lab/kirito/data/cliptrain/train/1_fake_noisy.csv')
parser.add_argument('--val_datapath', type=str,
                    default='/lab/kirito/data/GenImage/stable_diffusion_v_1_4/imagenet_ai_0419_sdv4/test_1.csv')
parser.add_argument("--continue_train", action='store_true')
parser.add_argument("--checkpoints", type=str, default='',
                    help="continue train model path")
parser.add_argument("--model", type=str, default='clip-adapter_rec1',
...
###### load data ######
train_dataset = thirdAugDataset(args.real_datapath, args.fake_datapath, args.realrec_datapath, args.fake_datapath)
c = train_dataset[0]
...
val_data_loader = torch.utils.data.DataLoader(val_dataset, batch_size=36, collate_fn=valDataset.collate_fn)

train_dataset_sizes = len(train_dataset)
temp = len(train_data_loader)
val_dataset_sizes = len(val_dataset)

device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')

# prepare the model (detector)
model_class = DETECTOR['CLIP_Adapter_rec']
...
def train(model, optimizer, num_epochs, start_epoch):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, data_dict in enumerate(train_data_loader):
            ...
            real_imgs, real_labels, fake_imgs, fake_labels, = data_dict['real_images'], data_dict[
                'real_labels'], data_dict['fake_images'], data_dict['fake_labels'],

            real_rec_imgs, real_rec_labels, fake_rec_imgs, fake_rec_labels = data_dict['real_rec_images'], data_dict[
                'real_rec_labels'], data_dict['fake_rec_images'], data_dict['fake_rec_labels']
            
            # with torch.set_grad_enabled(phase == 'train'):
            imgs_a = torch.cat((real_imgs, real_rec_imgs, fake_imgs), 0)
            labels_a = torch.cat((real_labels, real_rec_labels, fake_labels), 0)
            imgs_b = torch.cat([real_imgs, real_imgs, real_imgs], 0)
            labels_b = torch.cat([real_labels, real_labels, real_labels], 0)
            data_dict['fake_images'], data_dict['fake_labels'], data_dict['real_images'], data_dict[
                'real_labels'] = imgs_a.to(device), labels_a.to(device), imgs_b.to(device), labels_b.to(
                device)
