...
class testDataset(DeepfakeAbstractBaseDataset):
    def __init__(self, config=None, mode='test'):
        ...
        for im_path in self.image_list:
            ...
            domain = im_path.split('/')[6]
            if domain == 'youtube':
                self.label_spe_list.append(0)
                # print(self.label_spe_list)
            ...
            elif domain == 'frames' and im_path.split('/')[4] == 'Celeb-DF-v1':
                # if 'Celeb-real' in im_path:
                if 'YouTube-real' in im_path:
    ...
    def __getitem__(self, index):
        ...
        landmark_path = image_path.replace(
            'frames', 'landmarks').replace('.png', '.npy')
