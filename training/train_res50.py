...
parser.add_argument('--fake_datapath', type=str,
                    default='/lab/kirito/data/CNNspot_test/train/')
parser.add_argument('--real_datapath', type=str,
                    default='/lab/kirito/data/CNNspot_test/train/')
parser.add_argument("--continue_train", action='store_true')
parser.add_argument("--checkpoints", type=str, default='',
                    help="continue train model path")
parser.add_argument("--model", type=str, default='rest50',
...
dataloaders = {x: torch.utils.data.DataLoader(
    dataset=face_dataset[x], batch_size=args.batchsize, shuffle=True, num_workers=8,
    collate_fn=face_dataset[x].collate_fn) for x in ['train', 'val']}
dataset_sizes = {x: len(face_dataset[x]) for x in ['train', 'val']}
device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')

# prepare the model (detector)
model_class = DETECTOR['Res50Detector']
...
def train(model, optimizer, scheduler, num_epochs, start_epoch):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, data_dict in enumerate(dataloaders[phase]):
            ...
            imgs, labels = data_dict['image'], data_dict[
                'label']
...
def main():
    ...
    model, epoch = train(model, optimizer, exp_lr_scheduler, num_epochs=args.epochs, start_epoch=start_epoch)
