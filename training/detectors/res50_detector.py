...
@DETECTOR.register_module(module_name='Res50Detector')
class Res50Detector(AbstractDetector):
    def __init__(self):
        super().__init__()
    ...
    def build_backbone(self):
        ...
        backbone = ResNet50(config)
        '''backbone = backbone_class({'mode': 'original',
                                   'num_classes': 2, 'inc': 3, 'dropout': False})'''
        # if donot load the pretrained weights, fail to get good results
        state_dict = torch.load('/lab/kirito/clip-fairness/resnet50-19c8e357.pth')
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        # deal with acc
        _, prediction_class = torch.max(pred_dict['cls'], 1)
