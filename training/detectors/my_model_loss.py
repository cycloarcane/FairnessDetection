...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, phase):
        if phase == 'train':
            ...
            final_features_adapter = self.adapter2(image_features1)
...
@DETECTOR.register_module(module_name='CLIP_Fairness_loss')
class CLIP_Fairness_loss(nn.Module):
    def __init__(self, cfg):
        super(CLIP_Fairness_loss, self).__init__()
        self.cfg = cfg
        ...
        self.fake_prev_loss = [None] * 20
        self.real_prev_loss = [None] * 20
    ...
    def compute_loss(self, losslist, num):

        fake_weights = torch.ones(19)
        real_weights = torch.ones(19)
        ...
        for idx in range(20):
            if idx != num:
                if self.fake_prev_loss[idx] is not None and self.real_prev_loss[idx] is not None:
                    if part1[temp] > self.fake_prev_loss[idx]:
                        fake_weights[temp] = 1.0 + (part1[temp] / self.fake_prev_loss[idx]) * 0.1
                    else:
                        fake_weights[temp] = 1.0 - (self.fake_prev_loss[idx] / part1[temp]) * 0.1
                    
                    if part2[temp] > self.real_prev_loss[idx]:
                        real_weights[temp] = 1.0 + (part2[temp] / self.real_prev_loss[idx]) * 0.1
                    else:
                        real_weights[temp] = 1.0 - (self.real_prev_loss[idx] / part2[temp]) * 0.1
                    temp += 1
            
                else:
                    fake_weights[temp] = 1.0
                    real_weights[temp] = 1.0
                    temp += 1

        # fake_weights = fake_weights / torch.sum(fake_weights)
        # real_weights = real_weights / torch.sum(real_weights)

        index = 0
        for i in range(20):
            if i != num:
                self.fake_prev_loss[i] = part1[index]
                self.real_prev_loss[i] = part2[index]
                index += 1
        
        fake_weights_list = fake_weights.tolist()
        real_weights_list = real_weights.tolist()

        return fake_weights_list + real_weights_list
    
    
    def get_losses(self, data_dict, pred1, pred2, num):
        loss_list = []
        loss_list_new = []
        label = data_dict['fake_labels']
        loss_function = nn.CrossEntropyLoss(reduction='none')
        loss = loss_function(pred1, label)
        for i in range(len(loss)):
            loss_list.append(loss[i].item())

        for i in range(0, len(loss_list), 38):
            sublist = loss_list[i : i+38]
            weights = self.compute_loss(sublist,num)

            for idx in range(38):
                loss_list_new.append(sublist[idx] * weights[idx])

        loss_new = sum(loss_list_new) / len(loss_list_new)

        loss = torch.tensor(loss_new).to('cuda:1').requires_grad_()
        # loss1 = self.loss_func(pred1,label)
        loss2 = self.loss_func(pred2, label)
        loss_dict = {'firstloss': loss,
                     'finloss':loss2} 
        return loss_dict
